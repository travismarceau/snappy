...
extension Defaults {
    static func encoded() -> String? {
        ...
        let config = Config(bundleId: "com.travismarceau.snappy",
                            version: version,
                            shortcuts: shortcuts,
    ...
    static func loadFromSupportDir() {
        if let rectangleSupportURL = getSupportDir()?
            .appendingPathComponent("Snappy", isDirectory: true) {
            
            let configURL = rectangleSupportURL.appendingPathComponent("SnappyConfig.json")
                        
            let exists = try? configURL.checkResourceIsReachable()
            if exists == true {
                // Defense-in-depth: any process running as this user can drop
                // a SnappyConfig.json in Application Support and have it
                ...
                guard isSafe else {
                    AlertUtil.oneButtonAlert(
                        question: "Refused to load SnappyConfig.json",
                        text: "The configuration file at \(path) is a symlink or world-writable. Snappy has refused to load it. Remove the file or fix its permissions and try again."
                    )
                ...
                let response = AlertUtil.twoButtonAlert(
                    question: "Apply Snappy configuration?",
                    text: "A configuration file was found at \(path). Applying it will overwrite your current Snappy shortcuts and preferences. Apply now?",
                    confirmText: "Apply",
                    cancelText: "Discard"
                )
                ...
                do {
                    let newFilename = "SnappyConfig\(timestamp()).json"
                    ...
                    do {
                        ...
                        AlertUtil.oneButtonAlert(question: "Error after loading from Support Dir", text: "Unable to rename/remove SnappyConfig.json from \(rectangleSupportURL) after loading.")
                    }
                }
            }
        }
