...
class ShortcutManager {
    ...
    private func executeCycle(_ group: ShortcutCycle.Group) {
        ...
        let lastAction = AppDelegate.windowHistory.lastSnappyActions[windowId]
        ...
        let selectedAction = ShortcutCycle.action(
            in: group,
            lastAction: AppDelegate.windowHistory.lastSnappyActions[windowId],
    ...
    private func isRepeatAction(parameters: ExecutionParameters, windowElement: AccessibilityElement, windowId: CGWindowID) -> Bool {
        ...
        if parameters.action == AppDelegate.windowHistory.lastSnappyActions[windowId]?.action {
            return true
        }
        return false
    }
...
struct ShortcutCycle {
    ...
    static func action(in group: Group, lastAction: SnappyAction?, currentWindowRect: CGRect?) -> WindowAction {
    ...
    static func isStale(lastAction: SnappyAction?, currentWindowRect: CGRect?) -> Bool {
