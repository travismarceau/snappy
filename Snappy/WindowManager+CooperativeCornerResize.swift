...
extension WindowManager {
    ...
    func cooperativeCornerResizePlan(focusedWindowId: CGWindowID?,
                                     ...
                                     lastSnappyAction: SnappyAction?) -> CooperativeCornerApplicationPlan? {
        ...
        let isRepeatedCooperativeAction = action.isCompatibleRepeatedResizeAction(with: lastSnappyAction?.action)
    ...
    func applyCooperativeCornerCleanupIfNeeded(focusedWindowId: CGWindowID?,
                                               ...
                                               lastSnappyAction: SnappyAction?) {
        guard Defaults.cooperativeCornerResize.enabled,
              source.allowsCooperativeResize,
              let previousAction = lastSnappyAction?.action,
        ...
        guard let targetFrame = cleanupTargetFrame(action: previousAction,
                                                   ...
                                                   movedEdge: movedEdge,
                                                   tolerance: tolerance,
                                                   includeCycleTargets: (lastSnappyAction?.count ?? 0) > 1,
