# Divvtangle — Mac App Store submission

## What's already done in the codebase

- **Rebranded** to Divvtangle: bundle id `com.travismarceau.divvtangle`, display
  name, URL scheme `divvtangle://`, support dir `~/…/Application Support/Divvtangle`,
  config file `DivvtangleConfig.json`, all user-facing strings and the whole
  `Main.xcstrings` catalog, the app icon (`DivvtangleIcon`), the login-helper
  bundle id (`com.travismarceau.divvtangle.Launcher`). Internal Swift module stays
...
- **Sparkle removed entirely** (SPM package, code, Info.plist `SU*` keys, the
  ...
  App Store.
- **Release config = the MAS build**: `Divvtangle.app`, App Sandbox
...
- Debug config is the local dev build (ad-hoc, unsandboxed, `Divvtangle.app`).
...
2. **App Review Guideline 4.1 (Copycats).** Divvtangle is a rebranded fork of
   ...
   what's original.
3. `~/Library/Application Support/Divvtangle` under the sandbox resolves to the
...
1. **App Store Connect**: create the app record with bundle id
   `com.travismarceau.divvtangle`, name "Divvtangle", primary category
...
3. **Archive & upload**: Product → Archive (Release), then Organizer →
   ...
   `xcodebuild -scheme Rectangle -configuration Release archive -archivePath build/Divvtangle.xcarchive`
   then `xcodebuild -exportArchive -archivePath build/Divvtangle.xcarchive -exportOptionsPlist ExportOptions.plist -exportPath build/export`
...
5. **App Privacy**: Divvtangle collects no data — fill in "Data Not Collected".
...
7. **Accessibility usage**: the app needs the user to grant Accessibility; make
   ...
   Privacy & Security → Accessibility → enable Divvtangle).
