...
| File | Purpose |
|------|---------|
| `index.html` | The page content, illustrated desktop walkthrough, and small inline scripts for the compressing header and the walkthrough's final pause. |
| `style.css` | The monochrome layout, responsive styles, and motion. The walkthrough's desktop rules are kept together near the end. |
| `scroll.js` | Advances the desktop walkthrough as its text scrolls into view. |
| `appcast.xml` | Sparkle's update feed. Generated by `scripts/build-direct.sh` — never edit it by hand, it carries EdDSA signatures. |

The site has no build step or framework. The walkthrough text and initial desktop
remain visible without JavaScript; motion respects reduced-motion settings.
