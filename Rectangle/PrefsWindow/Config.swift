...
extension Defaults {
    static func encoded() -> String? {
        ...
        for defaultsKey in TodoManager.defaultsKeys + StackBadgeManager.defaultsKeys + PlacementModeManager.defaultsKeys {
            guard
        ...
        let config = Config(bundleId: "com.travismarceau.divvtangle",
                            version: version,
                            shortcuts: shortcuts,
    ...
    static func loadFromSupportDir() {
        if let rectangleSupportURL = getSupportDir()?
            .appendingPathComponent("Divvtangle", isDirectory: true) {
            
            let configURL = rectangleSupportURL.appendingPathComponent("DivvtangleConfig.json")
                        
            let exists = try? configURL.checkResourceIsReachable()
            if exists == true {
                // Defense-in-depth: any process running as this user can drop
                // a DivvtangleConfig.json in Application Support and have it
                ...
                guard isSafe else {
                    AlertUtil.oneButtonAlert(
                        question: "Refused to load DivvtangleConfig.json",
                        text: "The configuration file at \(path) is a symlink or world-writable. Divvtangle has refused to load it. Remove the file or fix its permissions and try again."
                    )
                ...
                let response = AlertUtil.twoButtonAlert(
                    question: "Apply Divvtangle configuration?",
                    text: "A configuration file was found at \(path). Applying it will overwrite your current Divvtangle shortcuts and preferences. Apply now?",
                    confirmText: "Apply",
                    cancelText: "Discard"
                )
                ...
                do {
                    let newFilename = "DivvtangleConfig\(timestamp()).json"
                    ...
                    do {
                        ...
                        AlertUtil.oneButtonAlert(question: "Error after loading from Support Dir", text: "Unable to rename/remove DivvtangleConfig.json from \(rectangleSupportURL) after loading.")
                    }
                }
            }
        }
